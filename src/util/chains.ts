...
function isTara(chainId: number): chainId is ChainId.TARAXA_TESTNET {
...
class TaraxaNativeCurrency extends NativeCurrency {
  ...
  get wrapped(): Token {
    if (!isTara(this.chainId)) throw new Error('Not taraxa');
    const nativeCurrency = WRAPPED_NATIVE_CURRENCY[this.chainId];
    if (nativeCurrency) {
      return nativeCurrency;
    }
  ...
  public constructor(chainId: number) {
    if (!isTara(chainId)) throw new Error('Not taraxa');
    super(chainId, 18, 'TARA', 'Taraxa');
  }
}
